        ...
        <form onSubmit={handleSubmit} className="px-8 py-5 space-y-7">
          <div className="grid grid-cols-2 gap-6">
            <div>
              ...
              <input id="nom" type="text" name="nom" value={formData.nom} onChange={handleChange} required className={inputClass} placeholder="Dupont" />
            </div>
            <div>
              <label htmlFor="prenom" className={labelClass}>Prénom *</label>
              <input id="prenom" type="text" name="prenom" value={formData.prenom} onChange={handleChange} required className={inputClass} placeholder="Jean" />
            </div>
          </div>

          <div>
            <label htmlFor="email" className={labelClass}>Email *</label>
            <input id="email" type="email" name="email" value={formData.email} onChange={handleChange} required className={inputClass} placeholder="jean@exemple.com" />
          </div>

          <div>
            <label htmlFor="telephone" className={labelClass}>Téléphone *</label>
            <input id="telephone" type="tel" name="telephone" value={formData.telephone} onChange={handleChange} required className={inputClass} placeholder="06 12 34 56 78" />
          </div>

          <div className="grid grid-cols-2 gap-6">
            <div>
              <label htmlFor="dateDebut" className={labelClass}>Date de début *</label>
              <input id="dateDebut" type="date" name="dateDebut" value={formData.dateDebut} onChange={handleChange} min={today} required className={inputClass} />
            </div>
            <div>
              <label htmlFor="dateFin" className={labelClass}>Date de fin *</label>
              <input id="dateFin" type="date" name="dateFin" value={formData.dateFin} onChange={handleChange} min={minDateFin} required className={inputClass} />
            </div>
          </div>

          <div>
            <label htmlFor="lieuPrise" className={labelClass}>Lieu de prise en charge</label>
            <input id="lieuPrise" type="text" name="lieuPrise" value={formData.lieuPrise} onChange={handleChange} className={inputClass} placeholder="Aéroport CDG, adresse Paris..." />
          </div>

          <div>
            <label htmlFor="message" className={labelClass}>Message / Demandes particulières</label>
            <textarea id="message" name="message" value={formData.message} onChange={handleChange} rows="1" className={`${inputClass} resize-none`} placeholder="GPS, siège bébé, assurance complémentaire..." />
          </div>
          ...
          <div className="border-t border-white/8 pt-6">
            <label className="flex items-start gap-4 cursor-pointer group">
              <div className="relative mt-[2px] shrink-0">
                <input type="checkbox" name="consentRgpd" checked={formData.consentRgpd} onChange={handleChange} required className="sr-only" />
                <div className={`w-4 h-4 border transition-colors duration-200 flex items-center justify-center ${formData.consentRgpd ? 'border-white/60 bg-white/10' : 'border-white/15 bg-transparent'}`}>
              ...
              <span className="text-[11px] font-light text-white/35 leading-relaxed">
                ...
                <a href="/pages/politique-confidentialite" target="_blank" className="text-white/55 hover:text-white transition-colors border-b border-white/20">
